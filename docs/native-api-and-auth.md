...
| Method | Path | Auth | 说明 |
|--------|------|------|------|
| `GET` | `/api/v1/repos/:owner/:repo/export` | admin | 导出完整仓库数据（threads + comments + reactions + labels） |

**查询参数**：`format`（`json`（默认）/ `csv`）、`since`（ISO 8601，仅导出此时间之后的数据）

**JSON 响应**（流式输出，大仓库不会 OOM）：
...
**CSV 格式**：按 thread 展开，每行一条 comment（含 thread 元数据列），适合导入电子表格。

**设计要点**：
- Admin 权限，防止普通用户批量拉取数据
- 数据库侧按 thread number 升序逐条查询，拼流式 JSON 写出，内存占用与总量无关
- `since` 参数用于增量备份：`WHERE updated_at >= ?1`
...
| `src/handlers/api/export.rs` | 数据导出 handler（JSON/CSV 流式输出） |
