...
use rand::rngs::OsRng;
use rsa::{RsaPrivateKey, pkcs1v15::SigningKey as RsaSigningKey, traits::PublicKeyParts};
use sha2::Sha256;
...
use super::{JwkKey, aud_matches, parse_jwt_parts, parse_max_age, verify_signature};
#[cfg(feature = "server")]
...
async fn verify_google_and_apple_wrappers_and_cache_paths() {
    ...
    let apple_user = verify_apple_id_token(&db, &apple_http, &apple_token, Some("apple-client"))
        .await
        .expect("apple verify");
